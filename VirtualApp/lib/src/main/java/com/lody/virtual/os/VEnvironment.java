...
public class VEnvironment {
    ...
    public static File getOdexFile(String packageName) {
        ...
        if (Build.VERSION.SDK_INT >= O) {
            ...
            odexFile = new File(oatDir, "base.odex");
        } else {
            odexFile =  new File(DALVIK_CACHE_DIRECTORY, "data@app@" + packageName + "-1@base.apk@classes.dex");
        }
        VLog.i(TAG, "OdexFile:%s exists:%s", odexFile.getPath(), odexFile.exists());
        return odexFile;
    }
